...
#	[string]$ScriptLogPath = Join-Path -Path $LogPath -ChildPath 'ScriptLogs'
	...
	#region  Set Logfile Name + Create List Array
		...
		If ($ExcludeGroups) {
			...
			If (($ExcludeGroups.Count) -gt 1) {
				[string]$InputDesc = "EXCLUDE - " + ($ExcludeGroups | Select -First 2) + " ..."
				[string]$InputItem = "EXCLUDE: " + ($ExcludeGroups | Select -First 2) + " ..."
			}
			Else {
				[string]$InputDesc = "EXCLUDE - " + $ExcludeGroups
	...
	#region Determine TimeZone
		...
		#  . "$SubScripts\Get-TimeZone_1.0.0.ps1"
	...
	# FILENAMES
	[string]$ResultsTextFileName = "Get-WSUSFailedClients_Results_" + $FileDateTime + $TimeZoneString + "_($InputDesc).log"
	[string]$ResultsCSVFileName = "Get-WSUSFailedClients_Results_" + $FileDateTime + $TimeZoneString + "_($InputDesc).csv"
