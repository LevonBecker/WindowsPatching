...
#region Tasks
	...
	#region Job Tasks
		...
		If ($Global:TestConnections.FailedCount -gt '0') {
			...
			[string]$FailedConnectResults = 'False,False,Unknown,0,0,Unknown,False' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + "Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,N/A,N/A,Failed Connection" + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
#							)
							...
							#region Output Results to File
								...
								[string]$TaskResults = $ComputerName + ',' + $CompleteSuccess + ',' + $AllPatchesInstalled + ',' + $DiskSpaceOK + ',' + $InstalledPatchesCount + ',' + $Global:RebootCount + ',' + $RebootNeeded + ',' +$ConnectSuccess + ',' + $Global:GetRuntime.Runtime + ',' + $JobStartTime + ',' + $Global:GetRuntime.EndTimeF + ',' + $OSVersion + ',' + $OSArch + ',' + $HostIP + ',' + $HostDomain + ',' + $DriveSize + ',' + $FreeSpace + ',' + $RebootRuntimes + ',' + $WuVBSExitCode + ',' + $ScriptErrors + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
#region Cleanup WIP
	...
	If ((Test-Path -Path "$WIPTempPath\*") -eq $true) {
		Get-Runtime -StartTime $ScriptStartTime
		[string]$TimedOutResults = 'False,False,Unknown,Unknown,Unknown,Unknown,True' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + "Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,Timed Out" + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
