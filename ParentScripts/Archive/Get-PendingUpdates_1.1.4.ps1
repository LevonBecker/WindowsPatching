...
.EXAMPLE
	Get-PendingUpdates -WsusGroups group01 
	...
	without spaces.
.EXAMPLE
	Get-PendingUpdates -WsusGroups group01,group02,group03 
	This will query the WSUS Server for a list of hostnames in a single or
	multiple WSUS groups to run the script against.
	This is an array so more than one can be listed as a comma seperated list
	without spaces.
...
#					}
	...
	# FILENAMES
	[string]$ResultsTextFileName = "Get-PendingUpdates_Results_" + $FileDateTime + $TimeZoneString + "_($InputDesc).log"
	[string]$ResultsCSVFileName = "Get-PendingUpdates_Results_" + $FileDateTime + $TimeZoneString + "_($InputDesc).csv"
