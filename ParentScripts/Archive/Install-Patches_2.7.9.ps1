...
#region Variables
	...
	# FILENAMES
	[string]$ResultsTextFileName = "Install-Patches_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).log"
	[string]$ResultsCSVFileName = "Install-Patches_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).csv"
...
#region Tasks
	...
	#region Job Tasks
		...
		If ($Global:TestConnections.FailedCount -gt '0') {
			...
			[string]$FailedConnectResults = 'False,False,Unknown,0,0,Unknown,False' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ' ' + $TimeZone + ',' + $Global:GetRuntime.EndTimeF + ' ' + $TimeZone + ',' + "Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,N/A,N/A,Failed Connection" + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
#															[Boolean]$RebootNeeded = $true
											...
											#region Install Patches
												...
												#region Reboot and Retry if Failed
...
#																[Boolean]$RebootNeeded = $true
														...
														Else {
#																	[Boolean]$RebootNeeded = $true
																	$ScriptErrors += 'Reboot Needed for VBS Failure '
														}
													} #Reboot if VBS Fails
											
												#endregion Reboot and Retry if Failed
											
											#endregion Install Patches
											
											#region Determine Patching Results
											...
											#endregion Determine Patching Results
											...
											If ($RunVBSSuccess -eq $true) {
												...
												#region Update History Logs and Reboot if Needed
													...
													If ($LocalLastPatchesLogfound -eq $true) {
														...
														# IF LATEST LOG NOT EMPTY DETERMINE IF REBOOT NEEDED
...
#																			[Boolean]$RebootNeeded = $true
												...
												#endregion Update History Logs and Reboot if Needed
										...
										Until (($AllPatchesInstalled -eq $true) -or ($PatchingRounds -ge 8) -or ($RebootFailed -eq $true) -or ($RemoteLastPatchesLogfound -eq $false)-or ($LastPatchesLogEmpty -eq $true) -or ($RunVBSSuccess -eq $false) -or ($NoRebootBail -eq $true))
...
#							)
							...
							#region Output Results to File
								...
								[string]$TaskResults = $ComputerName + ',' + $CompleteSuccess + ',' + $AllPatchesInstalled + ',' + $DiskSpaceOK + ',' + $InstalledPatchesCount + ',' + $Global:RebootCount + ',' + $RebootNeeded + ',' +$ConnectSuccess + ',' + $Global:GetRuntime.Runtime + ',' + $JobStartTime + ' ' + $TimeZone + ',' + $Global:GetRuntime.EndTimeF + ' ' + $TimeZone + ',' + $OSVersion + ',' + $OSArch + ',' + $HostIP + ',' + $HostDomain + ',' + $DriveSize + ',' + $FreeSpace + ',' + $RebootRuntimes + ',' + $WuVBSExitCode + ',' + $ScriptErrors + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
#region Cleanup WIP
	...
	If ((Test-Path -Path "$WIPTempPath\*") -eq $true) {
		Get-Runtime -StartTime $ScriptStartTime
		[string]$TimedOutResults = 'False,False,Unknown,Unknown,Unknown,Unknown,True' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ' ' + $TimeZone + ',' + $Global:GetRuntime.EndTimeF + ' ' + $TimeZone + ',' + "Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,Unknown,Timed Out" + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
2.7.2 - 12/13/2012
	...
	Switched to Func_Invoke-Patching 1.5.3
2.7.3 - 12/14/2012
