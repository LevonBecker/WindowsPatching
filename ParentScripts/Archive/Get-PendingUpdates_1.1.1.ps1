...
.OUTPUTS
	...
	-------------
	RESULTS
	%USERPROFILE%\Documents\Results\Get-PendingUpdates
	
	LOGS
	%USERPROFILE%\Documents\Logs\Get-PendingUpdates
	+---History
	+---JobData
	+---Latest
	+---WIP
...
.EXAMPLE
	Get-PendingUpdates -FileBrowser -SkipOutGrid
	FileBrowser:
		This switch will launch a separate file browser window.
		In the window you can browse and select a text or csv file from anywhere
		accessible by the local computer that has a list of host names.
...
.LINK
	http://wiki.bonusbits.com/main/PSScript:Get-PendingUpdates
...
#					  . "$SubScripts\Func_Reset-WPMUI_1.0.4.ps1"
					Reset-WPMUI -StartingWindowTitle $StartingWindowTitle -StartupVariables $StartupVariables -SkipPrompt
#				}
				Break
			}
			Else {
...
#region Variables
	...
	# DIRECTORY PATHS
	[string]$LogPath = ($Global:WindowsPatchingDefaults.GetPendingUpdatesLogPath)
	...
	[string]$ResultsPath = ($Global:WindowsPatchingDefaults.GetPendingUpdatesResultsPath)
...
#			}
	...
	# FILENAMES
	[string]$ResultsTextFileName = "Get-PendingUpdates_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).log"
	[string]$ResultsCSVFileName = "Get-PendingUpdates_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).csv"
...
#region Tasks
	...
	#region Job Tasks
		...
		If ($Global:TestConnections.FailedCount -gt '0') {
			...
			[string]$FailedConnectResults = 'False,False,Error,False' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + "Unknown,Unknown,Unknown,Unknown,Unknown,Failed Connection" + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
				...
				#region Background Job
					...
					Start-Job -ScriptBlock {
						...
						#region Job Variables
							...
							# HISTORY LOG
							[string]$HistoryLogFileName = $ComputerName + '_GetPendingUpdates_History.log' 
							...
							# LATEST LOG
							[string]$LatestLogFileName = $ComputerName + '_GetPendingUpdates_Latest.log' 
...
#									[string]$DriveSize = 'N/A'
							...
							If ($DiskSpaceOK -eq $true) {
								...
								#region Get Pending Updates
									...
									If ($Global:GetPendingPatches.Success -eq $true) {
										[boolean]$GetPendingUpdatesSuccess = $Global:GetPendingPatches.Success
										$PendingPatchCount = $Global:GetPendingPatches.PatchCount
										[string]$COMError = 'None'
										If ($PendingPatchCount -gt 0) {
											$PendingList = $null
											# Create Array Report Objects
											[array]$PendingList = $Global:GetPendingPatches.Report
											$KB = $null
									...
									Else {
										...
										$KBList = 'Error'
										[string]$COMError = ($Global:GetPendingPatches.Error | Out-String).Trim('')
									...
									# ADD RESULTS TO SCRIPT LOG ARRAY
									$results = $null
									[array]$results = ($Global:GetPendingPatches | Format-List | Out-String).Trim('')
									$results2 = $null
									[string]$results2 = ($Global:GetPendingPatches.Report | Format-List | Out-String).Trim('')
									$ScriptLogData += @(
										'',
						...
						#region Generate Report
							...
							#region Output Results to File
								...
								[string]$TaskResults = $ComputerName + ',' + $CompleteSuccess + ',' + $GetPendingUpdatesSuccess + ',' + $PendingPatchCount + ',' + $ConnectSuccess + ',' + $Global:GetRuntime.Runtime + ',' + $JobStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + $OSVersion + ',' + $OSArch + ',' + $HostIP + ',' + $HostDomain + ',' + $KBList + ',' + $ScriptErrors + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
#region Cleanup WIP
	...
	If ((Test-Path -Path "$WIPTempPath\*") -eq $true) {
		Get-Runtime -StartTime $ScriptStartTime
		[string]$TimedOutResults = 'False,False,Error,False' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + "Unknown,Unknown,Unknown,Unknown,Unknown,Timed Out" + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
