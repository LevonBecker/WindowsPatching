...
#region Variables
	...
	#region  Set Logfile Name + Create List Array
		...
		If (($Groups.Count) -gt 1) {
			[string]$HostInputDesc = "GROUPS - " + ($Groups | Select -First 2) + " ..."
			[string]$InputItem = "GROUPS: " + ($Groups | Select -First 2) + " ..."
		}
		Else {
			[string]$HostInputDesc = "GROUP - " + $Groups
	...
	# FILENAMES
	[string]$ResultsTextFileName = "Get-WSUSClients_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).log"
	[string]$ResultsCSVFileName = "Get-WSUSClients_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).csv"
