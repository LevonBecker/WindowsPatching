...
#>
	...
	Param (
		[parameter(Position=0,Mandatory=$true)][string]$UpdateServerURL,
		[parameter(Position=1,Mandatory=$true)][string]$UpdateServer,
	...
	#region Tasks
		...
		#region Create Log Directories
			...
			[string]$GetPendingUpdatesResultsPath = "$ResultsPath\Get-PendingUpdates"
			...
			If ((Test-Path -Path $GetPendingUpdatesResultsPath) -eq $false) {
				New-Item -Path $ResultsPath -Name 'Get-PendingUpdates' -ItemType Directory -Force | Out-Null
			}
			...
			$GetPendingUpdatesLogPath = Join-Path -Path $LogPath -ChildPath 'Get-PendingUpdates'
...
#			$GetWSUSClientsLogPath = Join-Path -Path $LogPath -ChildPath 'Get-WSUSClients'
			...
			If ((Test-Path -Path $GetPendingUpdatesLogPath) -eq $false) {
				New-Item -Path $LogPath -Name 'Get-PendingUpdates' -ItemType Directory -Force | Out-Null
			}
...
#			}
			...
			# CREATE SUB DIRECTORIES IF MISSING (Get-PendingUpdates)
			Foreach ($folder in $SubFolderList) {
				If ((Test-Path -Path "$GetPendingUpdatesLogPath\$folder") -eq $false) {
					New-Item -Path $GetPendingUpdatesLogPath -Name $folder -ItemType Directory -Force | Out-Null
				}
			}
...
#			}
	...
	$Global:WindowsPatchingDefaults = New-Object -TypeName PSObject -Property @{
		...
		GetPendingUpdatesLogPath = $GetPendingUpdatesLogPath
		GetPendingUpdatesResultsPath = $GetPendingUpdatesResultsPath
...
1.0.2 - 05/10/2012
	...
	Added NoTips
1.0.3 - 05/14/2012
	Added Get-PendingUpdates sections
1.0.3 - 05/15/2012
