...
#		[parameter(Mandatory=$true)][string]$SubScripts,
	...
	#region Tasks
		...
		If ($List) {
			...
			Test-Permissions -List $List -MaxJobs $MaxJobs -TestTimeout $TestTimeout -JobmonTimeout $JobmonTimeout -JobLogFullName $JobLogFullName -wintitle_input $Global:wintitle_input
